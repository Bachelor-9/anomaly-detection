...
def open_dataset_at_date(f: dict) -> pd.DataFrame:
    ...
    df['time'] = df['time'].dt.strftime('%Y-%m-%d %H:%M:%S') # This code was generated by ChatGPT (OpenAI)
...
def open_dataset_at_month(f: dict) -> pd.DataFrame:
    ...
    all_files = [os.path.join(path, file) for file in os.listdir(path) if file.endswith('.parquet')] # This code was generated by ChatGPT (OpenAI)
...
def produce_modified_dataset(df: pd.DataFrame, x_attr: list, y_attr: list) -> tuple:
    ...
    * Attributes attributes not present in neither ```x_attr``` and ```y_attr``` are removed.
    ...
    numeric_columns = df.select_dtypes(include=[np.number]) # This code was generated by ChatGPT (OpenAI)
    controlled_parameters = list(set(x_attr) & set(numeric_columns.columns.tolist())) # This code was generated by ChatGPT (OpenAI)
